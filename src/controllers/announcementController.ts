...
export async function listAnnouncementsAdmin(_req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
...
export async function reorderAnnouncements(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    await Promise.all(ids.map((id, index) => Announcement.updateOne({ _id: id }, { order: index })));
    res.json(await Announcement.find().sort({ order: 1, createdAt: -1 }));
  } catch (e) {
    next(e);
  }
